import { formatAddress, parseAddress, type Address, type Envelope } from '../protocol.js';
...
export async function runBridge(options: BridgeOptions): Promise<void> {
  ...
  try {
    while (options.shouldContinue?.() ?? true) {
      ...
      try {
        batch = await hub.inbox(cursor, waitMs);
      } catch (error) {
      ...
      for (const message of batch.data) {
        ...
        if (dest && dest.thread_id !== threadId) continue;
