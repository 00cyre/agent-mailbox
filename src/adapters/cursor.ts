import { formatAddress, replyEnvelope, type Envelope, type ReceiveResult, type SendResult, type VendorAdapter } from './protocol.js';
...
export class CursorAdapter implements VendorAdapter {
  ...
  #stashCliReply(threadId: string, original: Envelope, body: string): void {
    const list = this.#pending.get(threadId) ?? [];
    list.push({
      envelope: replyEnvelope(original, body, formatAddress('cursor', threadId)),
      seen: false,
      nativeId: 'cli-reply',
    });
    this.#pending.set(threadId, list);
  ...
  async #tryConversation(threadId: string): Promise<ReceiveResult> {
    ...
    for (const message of fresh) {
      ...
      if (original) {
        original.seen = true;
        envelopes.push(replyEnvelope(original.envelope, message.text, formatAddress('cursor', threadId)));
      } else {
        envelopes.push(
          replyEnvelope(
            {
              id: message.id ?? 'cursor-msg',
              from: formatAddress('cursor', threadId),
              to: formatAddress('cursor', threadId),
              reply_to: formatAddress('cursor', threadId),
              correlation_id: message.id ?? 'cursor-msg',
              body: message.text,
              created_at: new Date().toISOString(),
            },
            message.text,
            formatAddress('cursor', threadId)
          )
        );
  ...
  async #fromRuns(threadId: string): Promise<Envelope[]> {
    ...
    for (const run of payload.items ?? []) {
      ...
      const from = formatAddress('cursor', threadId);
      envelopes.push(
        original
  ...
  async #desktopSend(threadId: string, envelope: Envelope, text: string): Promise<SendResult> {
    try {
