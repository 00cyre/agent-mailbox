...
export class GrokAdapter implements ListeningVendorAdapter {
  ...
  #queueReply(threadId: string, original: Envelope, body: string): void {
    ...
    list.push(replyEnvelope(original, body, { vendor: 'grok', thread_id: threadId }));
      ...
      if (response.ok) {
        ...
        if (text && !this.#emitted.has(id)) {
          ...
          if (original) {
            original.seen = true;
            return {
              envelopes: [replyEnvelope(original.envelope, text, { vendor: 'grok', thread_id: threadId })],
              cursor: id,
            };
          }
          if (id !== cursor) {
            return {
              envelopes: [
                createEnvelope({
                  from: { vendor: 'grok', thread_id: threadId },
                  to: { vendor: 'grok', thread_id: threadId },
                  reply_to: { vendor: 'grok', thread_id: threadId },
                  correlation_id: id,
                  body: text,
                }),
              ],
