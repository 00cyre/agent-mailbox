...
import { formatForThread, parseFormatted, replyToFromRefs, refsWithReplyTo, replyEnvelope } from '../src/adapters/format.js';
import { envelopeFromMailbox, mailboxTarget } from '../src/adapters/format.js';
import { AdapterHub, envelopeFromSendBody } from '../src/adapters/dispatch.js';
...
const mail = (over: Partial<Parameters<typeof createEnvelope>[0]> = {}): Envelope =>
  createEnvelope({
    from: 'grok:resp_src',
    to: 'cursor:bc-dest',
...
describe('adapter protocol', () => {
  it('parses vendor:thread_id on the first colon', () => {
    assert.deepEqual(parseAddress('cursor:bc-abc:extra'), {
      vendor: 'cursor',
      thread_id: 'bc-abc:extra',
    });
    assert.equal(formatAddress({ vendor: 'grok', thread_id: 'resp_1' }), 'grok:resp_1');
    assert.equal(parseAddress('nocolon'), undefined);
  });
  ...
  it('defaults reply_to to the sender so replies have somewhere to land', () => {
    ...
    assert.deepEqual(env.reply_to, { vendor: 'cursor', thread_id: 'bc-1' });
    const reply = replyEnvelope(env, 'pong', { vendor: 'grok', thread_id: 'resp_2' });
    assert.deepEqual(reply.to, { vendor: 'cursor', thread_id: 'bc-1' });
    assert.deepEqual(reply.reply_to, { vendor: 'grok', thread_id: 'resp_2' });
  ...
  it('round-trips the pasted mailbox block', () => {
    ...
    assert.ok(parsed);
    assert.deepEqual(parsed.from, env.from);
    assert.deepEqual(parsed.to, env.to);
    assert.deepEqual(parsed.reply_to, env.reply_to);
...
describe('Cursor Cloud Agents', () => {
  it('follow-ups a bc-* thread and returns the run result to reply_to', async () => {
    ...
    const sent = await adapter.deliver('bc-dest', outbound);
    ...
    assert.equal(formatAddress(received.envelopes[0]!.to), 'grok:resp_src');
    assert.equal(formatAddress(received.envelopes[0]!.from), 'cursor:bc-dest');
  ...
  it('retries a 409 agent_busy follow-up', async () => {
    ...
    const sent = await adapter.deliver('bc-1', mail({ to: 'cursor:bc-1' }));
...
describe('Grok / xAI', () => {
  it('continues a Responses API thread and routes the model text to reply_to', async () => {
    ...
    const sent = await adapter.deliver('resp_src', outbound);
    assert.equal(sent.transport, 'xai');
    assert.equal(sent.nativeId, 'resp_next');

    const received = await adapter.receive('resp_src');
    assert.equal(received.envelopes.length, 1);
    assert.equal(formatAddress(received.envelopes[0]!.to), 'cursor:bc-1');
    assert.equal(formatAddress(received.envelopes[0]!.from), 'grok:resp_src');
...
describe('n-to-n bounce', () => {
  it('injects a grok reply back into the cursor thread named in reply_to', async () => {
    ...
    const cursor: ListeningVendorAdapter = {
      vendor: 'cursor',
      async send(_threadId, envelope) {
        delivered.push(envelope);
      },
      async deliver(_threadId, envelope): Promise<AdapterSendResult> {
        delivered.push(envelope);
    ...
    const grok: ListeningVendorAdapter = {
      vendor: 'grok',
      async send() {
        return;
      },
    ...
    assert.ok(bounced);
    assert.equal(formatAddress(delivered[0]!.to), 'cursor:bc-1');
    assert.equal(delivered[0]!.body, 'here is grok');
  });
});

describe('adapter HTTP', () => {
  class FakeCursor implements ListeningVendorAdapter {
    readonly vendor = 'cursor' as const;
    last: Envelope | undefined;
    async send(_threadId: string, envelope: Envelope): Promise<void> {
      await this.deliver(_threadId, envelope);
    }
    async deliver(_threadId: string, envelope: Envelope): Promise<AdapterSendResult> {
      this.last = envelope;
    ...
    async receive(): Promise<ReceiveResult> {
      ...
      return {
        envelopes: [replyEnvelope(this.last, 'ack from cursor', { vendor: 'cursor', thread_id: 'bc-9' })],
        cursor: '1',
      };
    }
  }
  ...
  it('accepts an envelope and serves the reply on receive', async () => {
    ...
    const payload = (await received.json()) as { data: { to: string }[] };
...
describe('mailbox mapping', () => {
  it('turns a hub message into vendor:thread addressing', () => {
    ...
    assert.equal(formatAddress(env.to), 'cursor:bc-xyz');
    assert.equal(formatAddress(env.from), 'alice:bc-xyz');
    assert.equal(formatAddress(env.reply_to!), 'alice:handshake');
